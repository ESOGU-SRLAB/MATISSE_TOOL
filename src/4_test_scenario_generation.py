...
default_prompts = {
    "deepseek-coder": "The main and only mission is 'GENERATING TEST SCENARIOS using information taking from other documents like python code, requirements, etc.' Create TEST SCENARIOS as a table (including rows and columns) using the given file as a reference, while taking into account the technical design, requirements specification, use case, traceability matrix, and test cases. Detect and refine any gaps or areas that require adjustments. The source code is: {source_code}. Technical Design: {tech_design}. Requirements Specification: {req_spec}. Use Case: {use_case}. Traceability Matrix: {trace_matrix}. Test Cases: {test_cases}. Prior Feedback: {feedback}",
    "codegemma": "The main and only mission is 'GENERATING TEST SCENARIOS using information taking from other documents like python code, requirements, etc.' Create TEST SCENARIOS as a table (including rows and columns) using the given file as a reference, while taking into account the technical design, requirements specification, use case, traceability matrix, and test cases. Detect and refine any gaps or areas that require adjustments. The source code is: {source_code}. Technical Design: {tech_design}. Requirements Specification: {req_spec}. Use Case: {use_case}. Traceability Matrix: {trace_matrix}. Test Cases: {test_cases}. Prior Feedback: {feedback}",
    "codellama": "The main and only mission is 'GENERATING TEST SCENARIOS using information taking from other documents like python code, requirements, etc.' Create TEST SCENARIOS as a table (including rows and columns) using the given file as a reference, while taking into account the technical design, requirements specification, use case, traceability matrix, and test cases. Detect and refine any gaps or areas that require adjustments. The source code is: {source_code}. Technical Design: {tech_design}. Requirements Specification: {req_spec}. Use Case: {use_case}. Traceability Matrix: {trace_matrix}. Test Cases: {test_cases}. Prior Feedback: {feedback}",
    "llama3.1": "The main and only mission is 'GENERATING TEST SCENARIOS using information taking from other documents like python code, requirements, etc.' Create TEST SCENARIOS as a table (including rows and columns) using the given file as a reference, while taking into account the technical design, requirements specification, use case, traceability matrix, and test cases. Detect and refine any gaps or areas that require adjustments. The source code is: {source_code}. Technical Design: {tech_design}. Requirements Specification: {req_spec}. Use Case: {use_case}. Traceability Matrix: {trace_matrix}. Test Cases: {test_cases}. Prior Feedback: {feedback}",
    "mathstral": "The main and only mission is 'GENERATING TEST SCENARIOS using information taking from other documents like python code, requirements, etc.' Create TEST SCENARIOS as a table (including rows and columns) using the given file as a reference, while taking into account the technical design, requirements specification, use case, traceability matrix, and test cases. Detect and refine any gaps or areas that require adjustments. The source code is: {source_code}. Technical Design: {tech_design}. Requirements Specification: {req_spec}. Use Case: {use_case}. Traceability Matrix: {trace_matrix}. Test Cases: {test_cases}. Prior Feedback: {feedback}"
}
...
st.title("Test Scenario Generation with Chain of Thought (CoT) and Prompt Chain")

st.write("""
Test Scenario Generation is a critical phase in the Software Test Life Cycle (STLC), where specific test scenarios are defined based on the requirements, design, and use cases of the software. This page facilitates the creation and management of your test scenarios using advanced techniques such as Chain of Thought (CoT) and Prompt Chain.""")

st.image("../images/test-scenario.jpg", caption="Test Scenario Generation Process")

st.subheader("Why Do We Generate Test Scenarios?")
st.write("""
- **Ensuring Comprehensive Test Coverage**: Test scenarios ensure that all functionalities, features, and user interactions are tested thoroughly.
- **Identifying Edge Cases**: By generating detailed scenarios, potential edge cases and exceptions can be identified and tested.
- **Aligning with Requirements**: Test scenarios are directly tied to the requirements and design documents, ensuring that all specified requirements are covered in the testing process.
- **Risk Mitigation**: By considering various scenarios, potential risks in the software can be identified early and mitigated through targeted testing.
- **Enhancing Test Efficiency**: Well-defined test scenarios help in streamlining the testing process, making it more efficient and effective.
""")

st.subheader("How Do We Generate Test Scenarios?")
st.write("""
The Test Scenario Generation process typically involves the following steps:

1. **Document Review**: Reviewing requirements specification, technical design, use case diagrams, and traceability matrix to understand the project’s scope and objectives.
2. **Defining Test Scenarios**: Outlining specific test scenarios based on the reviewed documents to ensure comprehensive coverage of all functionalities.
3. **Prioritizing Scenarios**: Determining the importance and risk associated with each scenario to prioritize testing efforts.
4. **Resource Allocation**: Identifying the resources, including personnel, tools, and environments, required to execute the test scenarios.
5. **Scenario Refinement**: Continuously refining and adjusting scenarios based on feedback from initial testing phases.
""")

st.subheader("Impact of Using Chain of Thought (CoT) and Prompt Chain in Test Scenario Generation")
st.write("""
Incorporating advanced techniques like Chain of Thought (CoT) and Prompt Chain into the Test Scenario Generation process can significantly enhance the thoroughness and effectiveness of the scenarios.

- **Chain of Thought (CoT)**: CoT enables a sequential reasoning approach, where the output of one scenario generation step informs the next. This method ensures that all aspects of the software, including edge cases, performance bottlenecks, and security vulnerabilities, are covered in the test scenarios.
  
- **Prompt Chain**: By using a series of prompts that guide the scenario generation process, Prompt Chain structures ensure that the scenarios are comprehensive and systematic. Each prompt is designed to focus on a specific aspect of the software, ensuring that no critical scenario is overlooked.

By combining these approaches, the Test Scenario Generation process becomes more robust, leading to scenarios that are better suited to ensure the quality and success of the software. The structured approach of CoT and Prompt Chains ensures that the scenarios are not only thorough but also adaptable to different testing requirements and conditions.
""")

st.subheader("Start Generating Test Scenarios with CoT and Prompt Chain")
st.write("""
The integration of advanced techniques like Chain of Thought (CoT) and Prompt Chain into the Test Scenario Generation process can significantly enhance the effectiveness and depth of the scenarios.

- **Python File**: The Python code file is essential for generating scenarios that test various aspects of the code, such as functionality, performance, and security. CoT enables a sequential reasoning approach, where the insights gained from code analysis shape the test scenarios.
""")
uploaded_code_file = st.file_uploader("Upload your Python code file (.py)", type="py")

st.write("""
- **Technical Design Document**: The Technical Design Document provides the architectural blueprint of the software. By analyzing this document, the Prompt Chain structures ensure that the test scenarios cover all critical design aspects, such as scalability, modularity, and adherence to design principles.
""")
uploaded_tech_design_file = st.file_uploader("Upload your Technical Design Document (.pdf, .docx, .txt)", type=["pdf", "docx", "txt"])

st.write("""
- **Requirements Specification Document**: The Requirements Specification Document outlines the functional and non-functional requirements of the software. In the Test Scenario Generation process, this document is essential for defining the scenarios that will verify that all requirements are met.
""")
uploaded_req_spec_file = st.file_uploader("Upload your Requirements Specification Document (.pdf, .docx, .txt)", type=["pdf", "docx", "txt"])

st.write("""
- **Use Case Diagrams and Stories**: These documents help to understand how the software is intended to be used. By analyzing these diagrams and stories, the Prompt Chain structures ensure that the test scenarios include all user interactions and real-world conditions.
""")
uploaded_use_case_file = st.file_uploader("Upload your Use Case Document (.pdf, .docx, .txt)", type=["pdf", "docx", "txt"])

st.write("""
- **Traceability Matrix**: The Traceability Matrix is crucial for ensuring that all requirements are linked to specific test scenarios. By cross-referencing the matrix with the requirements and design documents, the Prompt Chain structures help ensure that all scenarios are aligned with the project goals.
""")
uploaded_trace_matrix_file = st.file_uploader("Upload your Traceability Matrix (.pdf, .docx, .txt, .xlsx)", type=["pdf", "docx", "txt", "xlsx"])

st.write("""
- **Test Cases Document**: The Test Cases Document provides detailed steps for executing the test scenarios. By analyzing these test cases, the Prompt Chain structures ensure that the scenarios are comprehensive and that the software meets all specified requirements.
""")
uploaded_test_cases_file = st.file_uploader("Upload your Test Cases Document (.xlsx)", type=["pdf", "docx", "txt", "xlsx"])

st.write("""
By combining these approaches, the Test Scenario Generation process becomes more robust, leading to comprehensive scenarios that are better suited to ensure the quality and success of the software. The structured approach of CoT and Prompt Chains ensures that the scenarios are not only thorough but also adaptable to different testing requirements and conditions.
""")
    ...
    if (len(source_code) <= 10000 and 
        ...
        if use_cot:
            if st.button("Get Suggestion by LLaMa 3.1 Model"):
                ...
                MAIN PROMPT:
                Please pay attention: The only and main mission is GENERATING TEST SCENARIOS with duration times!
                ...
                {{
                    "suggestions": [
                        ...
                        "left_node": {{
                            ...
                            "prompt": "<Suggestion for Left Node> (Example: The mission is "GENERATING TEST SCENARIOS using information taking from other documents like python code, requirements, etc. (e.g., Create TEST SCENARIOS (must) as a table (including rows and columns) that shows the tasks and the time it will take to complete them based on the provided source code, technical design document, requirement specification, use case, traceability matrix and test cases. If there is feedback, apply corrections to the blank and required parts. The source code is: {source_code}. Technical Design: {tech_design}. Requirements Specification: {req_spec}. Use Case: {use_case}. Trace Matrix: {trace_matrix}. Test Cases: {test_cases},")
                        "right_node": {{
                            "llm_model": "<LLM Model Name> (e.g., codellama)",
                            "prompt": "<Suggestion for Right Node> (Example: The mission is "GENERATING TEST SCENARIOS using information taking from other documents like python code, requirements, etc. (e.g., Create TEST SCENARIOS (must) as a table (including rows and columns) that shows the tasks and the time it will take to complete them based on the provided source code, technical design document, requirement specification, use case, traceability matrix and test cases. If there is feedback, apply corrections to the blank and required parts. The source code is: {source_code}. Technical Design: {tech_design}. Requirements Specification: {req_spec}. Use Case: {use_case}. Trace Matrix: {trace_matrix}. Test Cases: {test_cases}")
                        }}
                        }},
            ...
            if st.session_state.suggestion_output:
                ...
                if st.button("Apply the Suggested Structure"):
                    ...
                    suggested_json_structure = extract_and_save_json(st.session_state.suggestion_output, "generated_test_scenario_suggested_structure.json")
            ...
            if st.session_state.apply_suggestion:
                try:
                    with open('generated_test_scenario_suggested_structure.json', 'r') as suggested_json_file:
                    ...
                    st.error("The file 'generated_test_scenario_suggested_structure.json' was not found.")
                except json.JSONDecodeError:
                    st.error("Failed to decode JSON from 'generated_test_scenario_suggested_structure.json'.")
