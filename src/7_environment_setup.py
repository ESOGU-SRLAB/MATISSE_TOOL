...
default_prompts = {
    "deepseek-coder": "Based on the following inputs, provide detailed instructions for setting up the test environment. Inputs are: {input_content}. Include hardware and software requirements, system configurations, network settings, test tools and frameworks, and environmental dependencies and integrations.",
    "codegemma": "Analyze the given inputs to provide comprehensive setup instructions for the test environment. The inputs are: {input_content}. Consider hardware and software requirements, system configurations, network settings, test tools and frameworks, and environmental dependencies and integrations. Focus on accuracy and completeness.",
    "codellama": "Review the inputs to create detailed setup instructions for the test environment. The inputs are: {input_content}. Incorporate details on hardware and software requirements, system configurations, network settings, test tools and frameworks, and environmental dependencies and integrations. Provide detailed recommendations.",
    "llama3.1": "Evaluate the inputs to generate comprehensive setup instructions for the test environment. The inputs are: {input_content}. Summarize strengths, weaknesses, and suggest improvements based on hardware and software requirements, system configurations, network settings, test tools and frameworks, and environmental dependencies and integrations.",
    "mathstral": "Review the inputs to provide efficient and detailed setup instructions for the test environment. The inputs are: {input_content}. Include insights on hardware and software requirements, system configurations, network settings, test tools and frameworks, and environmental dependencies and integrations. Suggest enhancements for accuracy and completeness."
}
...
st.write("This application allows users to configure an LLM-based system for setting up test environments using various models.")

# File upload for environment setup
uploaded_file = st.file_uploader("Upload your Python code file (.py)", type="py")

# Additional inputs for environment setup
st.write("### Additional Inputs for Environment Setup")
hardware_requirements = st.text_area("Enter hardware requirements (if any):", "")
software_requirements = st.text_area("Enter software requirements (if any):", "")
system_configurations = st.text_area("Enter system configurations and network settings (if any):", "")
test_tools = st.text_area("Enter test tools and frameworks (if any):", "")
environmental_dependencies = st.text_area("Enter environmental dependencies and integrations (if any):", "")
...
if uploaded_file is not None:
    ...
    if len(code_content) <= 15000:
        st.text_area("Uploaded Python Code", code_content, height=300)
        ...
        if use_cot:
            if st.button("Get Suggestion by LLaMa 3.1 Model"):    
                ...
                combined_inputs = f"Python Code: {code_content}\nHardware Requirements: {hardware_requirements}\nSoftware Requirements: {software_requirements}\nSystem Configurations: {system_configurations}\nTest Tools: {test_tools}\nEnvironmental Dependencies: {environmental_dependencies}"
        ...
        if st.session_state.review_completed:
            ...
            for i in range(st.session_state.depth):
                if st.button(f"Save Outputs to txt (Level {i + 1})", key=f"save_txt_level_{i}"):
                    ...
                    txt_filename = f"environment_setup_outputs_level_{i + 1}.txt"
                ...
                if st.button(f"Save Outputs to JSON (Level {i + 1})", key=f"save_json_level_{i}"):
                    ...
                    json_filename = f"environment_setup_outputs_level_{i + 1}.json"
            ...
            if st.button("Save Final Outputs to txt"):
                ...
                txt_filename = "final_environment_setup_outputs.txt"
                with open(txt_filename, 'w') as txt_file:
            ...
            if st.button("Save Final Outputs to JSON"):
                st.session_state.final_output_saved = True
                json_filename = "final_environment_setup_outputs.json"
                with open(json_filename, 'w') as json_file:
    ...
    st.warning("Please upload a Python code file to proceed.")
