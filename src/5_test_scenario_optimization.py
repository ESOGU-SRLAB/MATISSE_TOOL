...
default_prompts = {
    "deepseek-coder": "Based on the following inputs, optimize the existing test scenarios to increase their effectiveness. Inputs are: {input_content}. Include insights from existing test scenarios and results, performance and load test results, user feedback and priorities, and risk analysis and bug reports.",
    "codegemma": "Analyze the given inputs to optimize the existing test scenarios for better effectiveness. The inputs are: {input_content}. Consider information from existing test scenarios and results, performance and load test results, user feedback and priorities, and risk analysis and bug reports. Focus on eliminating redundant steps and highlighting critical tests.",
    "codellama": "Review the inputs to optimize the existing test scenarios, improving their effectiveness. The inputs are: {input_content}. Incorporate details from existing test scenarios and results, performance and load test results, user feedback and priorities, and risk analysis and bug reports. Provide detailed recommendations.",
    "llama3.1": "Evaluate the inputs to optimize the test scenarios for better performance and efficiency. The inputs are: {input_content}. Summarize strengths, weaknesses, and suggest improvements based on existing test scenarios and results, performance and load test results, user feedback and priorities, and risk analysis and bug reports.",
    "mathstral": "Review the inputs to optimize the test scenarios, ensuring logical and comprehensive coverage while eliminating redundant steps. The inputs are: {input_content}. Include insights from existing test scenarios and results, performance and load test results, user feedback and priorities, and risk analysis and bug reports. Suggest enhancements for thoroughness and efficiency."
}
...
st.title("Test Scenario Optimization")
st.write("This application allows users to configure an LLM-based system for optimizing test scenarios using various models.")

# File upload for test scenario optimization
uploaded_file = st.file_uploader("Upload your existing test scenarios (.txt)", type="txt")

# Additional inputs for test scenario optimization
st.write("### Additional Inputs for Test Scenario Optimization")
performance_results = st.text_area("Enter performance and load test results (if any):", "")
user_feedback = st.text_area("Enter user feedback and priorities (if any):", "")
risk_analysis = st.text_area("Enter risk analysis and bug reports (if any):", "")
...
if uploaded_file is not None:
    ...
    test_scenarios_content = uploaded_file.read().decode('utf-8')
    if len(test_scenarios_content) <= 15000:
        st.text_area("Uploaded Test Scenarios Content", test_scenarios_content, height=300)
        ...
        if use_cot:
            if st.button("Get Suggestion by LLaMa 3.1 Model"):    
                ...
                combined_inputs = f"Test Scenarios: {test_scenarios_content}\nPerformance Results: {performance_results}\nUser Feedback: {user_feedback}\nRisk Analysis: {risk_analysis}"
        ...
        if st.session_state.review_completed:
            ...
            for i in range(st.session_state.depth):
                if st.button(f"Save Outputs to txt (Level {i + 1})", key=f"save_txt_level_{i}"):
                    ...
                    txt_filename = f"test_scenario_optimization_outputs_level_{i + 1}.txt"
                ...
                if st.button(f"Save Outputs to JSON (Level {i + 1})", key=f"save_json_level_{i}"):
                    ...
                    json_filename = f"test_scenario_optimization_outputs_level_{i + 1}.json"
            ...
            if st.button("Save Final Outputs to txt"):
                ...
                txt_filename = "final_test_scenario_optimization_outputs.txt"
                with open(txt_filename, 'w') as txt_file:
            ...
            if st.button("Save Final Outputs to JSON"):
                st.session_state.final_output_saved = True
                json_filename = "final_test_scenario_optimization_outputs.json"
                with open(json_filename, 'w') as json_file:
    ...
    st.warning("Please upload a test scenarios file to proceed.")
