...
default_prompts = {
    "deepseek-coder": "Based on the following inputs, prepare a detailed test closure report. Inputs are: {input_content}. Include verification of all test results, error corrections, user approvals and feedback, and final test and quality control reports.",
    "codegemma": "Analyze the given inputs to prepare a comprehensive test closure report. The inputs are: {input_content}. Consider all test results, error corrections, user approvals and feedback, and final test and quality control reports. Focus on completeness and accuracy.",
    "codellama": "Review the inputs to create a detailed test closure report. The inputs are: {input_content}. Incorporate verification of all test results, error corrections, user approvals and feedback, and final test and quality control reports. Provide detailed recommendations.",
    "llama3.1": "Evaluate the inputs to generate a comprehensive test closure report. The inputs are: {input_content}. Summarize strengths, weaknesses, and suggest improvements based on all test results, error corrections, user approvals and feedback, and final test and quality control reports.",
    "mathstral": "Review the inputs to provide an efficient and detailed test closure report. The inputs are: {input_content}. Include verification of all test results, error corrections, user approvals and feedback, and final test and quality control reports. Suggest enhancements for completeness and accuracy."
}
...
st.title("Test Closure")
st.write("This application allows users to configure an LLM-based system for generating detailed test closure reports using various models.")

# File upload for test results
uploaded_file = st.file_uploader("Upload your test results file (.txt, .json)", type=["txt", "json"])

# Additional inputs for test closure
st.write("### Additional Inputs for Test Closure")
error_correction_reports = st.text_area("Enter error correction and improvement reports (if any):", "")
user_feedback = st.text_area("Enter user approvals and feedback (if any):", "")
final_quality_reports = st.text_area("Enter final test and quality control reports (if any):", "")
...
if uploaded_file is not None:
    ...
    combined_inputs = f"Test Results: {test_results}\nError Correction Reports: {error_correction_reports}\nUser Feedback: {user_feedback}\nFinal Quality Reports: {final_quality_reports}"
    ...
    if st.session_state.review_completed:
        ...
        for i in range(st.session_state.depth):
            if st.button(f"Save Outputs to txt (Level {i + 1})", key=f"save_txt_level_{i}"):
                ...
                txt_filename = f"test_closure_outputs_level_{i + 1}.txt"
            ...
            if st.button(f"Save Outputs to JSON (Level {i + 1})", key=f"save_json_level_{i}"):
                ...
                json_filename = f"test_closure_outputs_level_{i + 1}.json"
        ...
        if st.button("Save Final Outputs to txt"):
            ...
            txt_filename = "final_test_closure_outputs.txt"
            with open(txt_filename, 'w') as txt_file:
        ...
        if st.button("Save Final Outputs to JSON"):
            st.session_state.final_output_saved = True
            json_filename = "final_test_closure_outputs.json"
            with open(json_filename, 'w') as json_file:
