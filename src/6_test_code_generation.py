...
default_prompts = {
    "deepseek-coder": "Based on the following inputs, generate detailed test code for the provided test scenarios. Inputs are: {input_content}. Include insights from test scenarios and specifications, existing codebase and API documentation, test tools and frameworks, and coding standards and best practices.",
    "codegemma": "Analyze the given inputs to generate test code for the provided test scenarios. The inputs are: {input_content}. Consider information from test scenarios and specifications, existing codebase and API documentation, test tools and frameworks, and coding standards and best practices. Focus on error detection and code optimization.",
    "codellama": "Review the inputs to create executable test code for the provided test scenarios. The inputs are: {input_content}. Incorporate details from test scenarios and specifications, existing codebase and API documentation, test tools and frameworks, and coding standards and best practices. Provide detailed recommendations.",
    "llama3.1": "Evaluate the inputs to generate comprehensive test code for the provided test scenarios. The inputs are: {input_content}. Summarize strengths, weaknesses, and suggest improvements based on test scenarios and specifications, existing codebase and API documentation, test tools and frameworks, and coding standards and best practices.",
    "mathstral": "Review the inputs to generate efficient and detailed test code for the provided test scenarios. The inputs are: {input_content}. Include insights from test scenarios and specifications, existing codebase and API documentation, test tools and frameworks, and coding standards and best practices. Suggest enhancements for error detection and code optimization."
}
...
st.title("Test Code Generation")
st.write("This application allows users to configure an LLM-based system for generating test code using various models.")

# File upload for test code generation
uploaded_file = st.file_uploader("Upload your test scenarios (.txt)", type="txt")

# Additional inputs for test code generation
st.write("### Additional Inputs for Test Code Generation")
existing_codebase = st.text_area("Enter existing codebase and API documentation (if any):", "")
test_tools = st.text_area("Enter test tools and frameworks (if any):", "")
coding_standards = st.text_area("Enter coding standards and best practices (if any):", "")
        ...
        if st.session_state.review_completed:
            ...
            for i in range(st.session_state.depth):
                if st.button(f"Save Outputs to txt (Level {i + 1})", key=f"save_txt_level_{i}"):
                    ...
                    txt_filename = f"test_code_generation_outputs_level_{i + 1}.txt"
                ...
                if st.button(f"Save Outputs to JSON (Level {i + 1})", key=f"save_json_level_{i}"):
                    ...
                    json_filename = f"test_code_generation_outputs_level_{i + 1}.json"
            ...
            if st.button("Save Final Outputs to txt"):
                ...
                txt_filename = "final_test_code_generation_outputs.txt"
                with open(txt_filename, 'w') as txt_file:
            ...
            if st.button("Save Final Outputs to JSON"):
                st.session_state.final_output_saved = True
                json_filename = "final_test_code_generation_outputs.json"
                with open(json_filename, 'w') as json_file:
